/**
 * Populates the Postgres database with starting data: organizations,
 * rules, an adjuster roster, a sample policy document (chunked and indexed
 * for retrieval, same code path a real upload uses), and a handful of demo
 * claims run through the actual intake pipeline (fraud model, rules
 * engine, coverage assistant, smart assignment) so the audit trail on each
 * one is genuine, not hand-written.
 *
 * Requires DATABASE_URL to be set (.env.local for local dev) and
 * supabase/schema.sql to already be applied to that database.
 *
...
async function main() {
  ...
  for (const org of ORGANIZATIONS) {
    await pool.query(
      ...
      [
        ...
        org.sla_hours,
        org.fraud_threshold,
        org.high_value_threshold,
        JSON.stringify(org.required_documents),
        org.brand_color,
        org.claims_source,
        org.policy_source,
      ]
  ...
  const users = [
    { org: "org_acme", name: "Jamie Alvarez", role: "senior_adjuster", email: "jamie@acme.example" },
    { org: "org_acme", name: "Rae Kim", role: "adjuster", email: "rae@acme.example" },
    { org: "org_acme", name: "Priya Shah", role: "fde", email: "priya@acme.example" },
    { org: "org_acme", name: "Dana Cho", role: "executive", email: "dana@acme.example" },
    { org: "org_voyage", name: "Tunde Osei", role: "adjuster", email: "tunde@voyage.example" },
    { org: "org_voyage", name: "Sam Ruiz", role: "executive", email: "sam@voyage.example" },
  ];
  for (const u of users) {
    await pool.query(
      "INSERT INTO users (id, organization_id, name, role, email) VALUES ($1,$2,$3,$4,$5)",
      [randomUUID(), u.org, u.name, u.role, u.email]
    );
  }
...
`;
  const acmeDocId = await addPolicyDocument(
    "org_acme",
    null,
    "acme-commercial-property-CP-2200.txt",
    acmePolicyText
  );
  const acmeChunks = chunkText(acmePolicyText);
  for (let i = 0; i < acmeChunks.length; i++) {
    await addDocumentChunk(acmeDocId, i, acmeChunks[i], termFrequencies(acmeChunks[i]));
  }
  ...
  for (const dc of demoClaims) {
    ...
    await runIntakePipeline(claim);

    // Backdate created_at + every event for this claim by the same offset,
    // so the SLA table on the executive dashboard shows a realistic mix of
    // on-track/at-risk/breached claims, while preserving the real spacing
    // between pipeline steps.
    const shiftMs = backdateHours * 3600 * 1000;
    const backdatedCreatedAt = new Date(Date.now() - shiftMs).toISOString();
    await pool.query("UPDATE claims SET created_at = $1 WHERE id = $2", [backdatedCreatedAt, claim.id]);

    const { rows: events } = await pool.query(
      "SELECT id, timestamp FROM claim_events WHERE claim_id = $1",
      [claim.id]
    );
    for (const ev of events) {
      const shifted = new Date(new Date(ev.timestamp).getTime() - shiftMs).toISOString();
      await pool.query("UPDATE claim_events SET timestamp = $1 WHERE id = $2", [shifted, ev.id]);
    }
  }

  console.log("Done. Database is ready.");
  console.log(
    `Organizations: ${ORGANIZATIONS.length}, Rules: ${RULES.length}, Adjusters: ${adjusters.length}, Claims: ${demoClaims.length}`
  );
